...
class CompanyAddress
  ...
  def self.create(company_id:, zip:, street:, city:, state:)
    ...
    return "O cep é obrigatório" if new_address.zip.blank?
    return "O estado é obrigatório" if new_address.state.blank?
    return "A cidade é obrigatória" if new_address.city.blank?
    return "A rua é obrigatória" if new_address.street.blank?
